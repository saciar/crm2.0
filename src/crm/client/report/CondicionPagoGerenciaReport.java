...
public class CondicionPagoGerenciaReport extends CongressReportService implements CondicionPagoGerenciaReportSEI {
	private static final String REPORT_PPTOS_GCIA_NAME = "jasper/CondicionPagoGerencia.jasper";
	
	private CondicionPagoGerenciaReportSEI stub;
	private static CondicionPagoGerenciaReport instance;
	
	public CondicionPagoGerenciaReport() throws WSIFException{
		super("CondicionPagoGerenciaReportSEI");
	}
	
	protected void registerTypes() throws WSIFException {
		// map types
		try {
			addType("CondicionPagoGerencia", Class.forName("crm.libraries.report.CondicionPagoGerencia"));

		} catch (ClassNotFoundException e) {
			e.printStackTrace();
		}
		
		stub = (CondicionPagoGerenciaReportSEI)service.getStub(CondicionPagoGerenciaReportSEI.class);
	}
	
	public static CondicionPagoGerenciaReport instance() {
		try {
			if (instance == null) {
				instance = new CondicionPagoGerenciaReport();
			}
	...
	public JasperPrint createPresupuestosGciaReport(DatePanel dateInicio, DatePanel dateFin, long codVendedor, long codUC, long codLugar, int type) throws RemoteException, JRException {
		...
		CondicionPagoGerencia[] presupuestos = null;
	...
	private JasperPrint createReport(CondicionPagoGerencia[] eventos, String title, String subTitle, int year, 
		...
		if(isLugarUsed(type)){
			...
			PrtPptoCondPago c = PrtPptoCondPagoManager.instance().getById(String.valueOf(codCondPago));
			parameters.put("CONDPAGO", c.getTitulo());
		}	
		// 3- create JasperPrint using fillReport() method
		JasperPrint jasperPrint = null;
		
		jasperPrint = JasperFillManager.fillReport(eventReport, parameters, CondPagoGerenciaReportDSBuilder.toJRMap(eventos, codCondPago));
		
		return jasperPrint;
	}

	public CondicionPagoGerencia[] findByRangeDate(int day1, int month1, int year1, int day2, int month2, int year2, long codVendedor, long codUC, long codLugar) throws RemoteException {
...
class CondPagoGerenciaReportDSBuilder{
	
	private static double totalFacturado;
	
	@SuppressWarnings("unchecked")
	public static JRMapArrayDataSource toJRMap(CondicionPagoGerencia[] eventos, long codCondPago){
		Object[] data = new Object[eventos.length];
		int totalPptos = 0;
		int totalOtros = 0;
		int j=0;
		for (int i=0;i<eventos.length;i++) {			
			CondicionPagoGerencia evento = eventos[i];
			if(evento.getNroPpto() != 0){
				...
				map.put("condicion", evento.getCondPago());
				map.put("codCondPago", codCondPago);
