...
public class PresupuestosConfirmadosGerenciaReport extends CongressReportService implements PresupuestosConfirmadosGerenciaReportSEI{

	private static final String REPORT_PPTOS_GCIA_NAME = "jasper/PresupuestosConfirmadosGerencia.jasper";
	
	private PresupuestosConfirmadosGerenciaReportSEI stub;
	private static PresupuestosConfirmadosGerenciaReport instance;
	
	public PresupuestosConfirmadosGerenciaReport() throws WSIFException{
		super("PresupuestosConfirmadosGerenciaReportSEI");
	}
	
	protected void registerTypes() throws WSIFException {
		// map types
		try {
			addType("PresupuestosConfirmadosGerencia", Class.forName("crm.libraries.report.PresupuestosConfirmadosGerencia"));

		} catch (ClassNotFoundException e) {
			e.printStackTrace();
		}
		
		stub = (PresupuestosConfirmadosGerenciaReportSEI)service.getStub(PresupuestosConfirmadosGerenciaReportSEI.class);
	}
	
	public static PresupuestosConfirmadosGerenciaReport instance() {
		try {
			if (instance == null) {
				instance = new PresupuestosConfirmadosGerenciaReport();
			}
...
public JasperPrint createPresupuestosGciaReport2(DatePanel dateInicio, DatePanel dateFin) throws RemoteException, JRException {
		...
		PresupuestosConfirmadosGerencia[] presupuestos = findByRangeDate(day1, month1, year1, day2, month2, year2);
	...
	private JasperPrint createReport(PresupuestosConfirmadosGerencia[] eventos, String title, String subTitle, int year) throws RemoteException, JRException {
		...
		JasperPrint jasperPrint = JasperFillManager.fillReport(eventReport, parameters, PptoConfirmadosGerenciaReportDSBuilder.toJRMap(eventos));
		
		return jasperPrint;
	}

	public PresupuestosConfirmadosGerencia[] findByRangeDate(int day1, int month1, int year1, int day2, int month2, int year2) throws RemoteException {
...
class PptoConfirmadosGerenciaReportDSBuilder{
	
	private static double totalFacturado;
	
	public static JRMapArrayDataSource toJRMap(PresupuestosConfirmadosGerencia[] eventos){
		Object[] data = new Object[eventos.length];
		
		for (int i=0;i<eventos.length;i++) {
			PresupuestosConfirmadosGerencia evento = eventos[i];
			Map map = new HashMap();
