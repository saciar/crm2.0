...
public class PantallaEditarCliente extends PantallaEmergente{
    ...
    public PantallaEditarCliente(Frame owner){
		super("Editar datos del cliente",owner);
	...
	private void resetFields() {
		...
		if(!Util.isBlank(this.cliente.getPagoContacto())){
			m_txtPagoContacto.setText(this.cliente.getPagoContacto());
			jLabel9.setForeground(Color.black);
		}
		else
			jLabel9.setForeground(Color.red);
		if(!Util.isBlank(this.cliente.getPagoTelefono())){
		...
		if(!Util.isBlank(this.cliente.getIva())){
		...
		if(domicio != null && !Util.isBlank(domicio)){
			...
			m_txtNroDomicilioPago.setText(domicio.substring(domicio.lastIndexOf(" ")+1, domicio.length()));
			jLabel32.setForeground(Color.black);
		}
		else
			jLabel32.setForeground(Color.red);
		
		if(!Util.isBlank(this.clienteFacturacion.getDiaHoraPago())){
    ...
    private boolean isFormIncomplete() {
		...
		return emp == null || emp.length() == 0 || 
				...
				|| facCalle == null || facCalle.length() == 0
				|| facNumero == null || facNumero.length() == 0
				|| facLocalidad == null || facLocalidad.length() == 0
	...
	public void btnAgregarActionPerformed(java.awt.event.ActionEvent evt) {
		...
		if (isFormIncomplete()) {
			Util.errMsg(PantallaEditarCliente.this, "Tiene datos obligatorios por completar", null);
			return;
		}
		...
		try {
			...
			if (!StringUtils.isBlank(clienteID)) {
				...
				ClienteFacturacionManager.instance().update(this.clienteFacturacion);
    ...
    private class SalirAction implements ActionListener {
		public void actionPerformed (ActionEvent evt) {
			if (MessageUtil.showYesNoMessage(PantallaEditarCliente.this, "¿Desea salir sin grabar el cliente?", "Salir")){
				setVisible(false);				
			}
		}
	}
