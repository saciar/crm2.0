...
public class BuscadorPptos extends PantallaEmergente{
	...
	public BuscadorPptos(Frame owner){		
	...
	public void initComponent(){	
        ...
        duplicar = new GradientButton("", Color.blue);
        ...
        duplicar.setIcon(new javax.swing.ImageIcon(getUrlImagen("application_double.png")));
        duplicar.setMnemonic('d');
        duplicar.setText("Duplicar");
        duplicar.setToolTipText("Click para duplicar el presupuesto seleccionado");
	...
	private void createListener(){
		...
		duplicar.addActionListener(new DuplicarActionListener());
	...
	private void loadPresupuestoDuplicado(final Presupuesto p) {	
		MainPanelComercial gp = PantallaBienvenidaVerdadera.getMainPanel2();								
		gp.setPresupuesto(Presupuesto.createCopia(pptoElegido));
		cambiarPantallaA(gp);		
	}
	...
	private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(BuscadorPptos.this, "¿Desea salir de mis presupuestos?", "Salir")){
				dispose();
			}
		}
	...
	private class AvanzadaActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			NuevaBusquedaAvanzada buscaPpto= new NuevaBusquedaAvanzada(BuscadorPptos.this);
		
				buscaPpto.initComponents();
				buscaPpto.initLayout();
				buscaPpto.setOwner(BuscadorPptos.this);
	...
	private class DuplicarActionListener implements ActionListener{
								...
								if (item != null) {
									try {
										...
										if(pptoElegido.getVendedor().getVendedor().getCodUsuario().equals(getUser().getCodigo()))
											...
											loadPpto=false;
											Util.alertMsg(Main.getVentana(), "El presupuesto es de otro usuario.\nSolo "+pptoElegido.getVendedor().getVendedor().getApellidoYNombre()+" puede duplicarlo.");
										}
									...
									if(loadPpto){
										setVisible(false);
										loadPresupuestoDuplicado(pptoElegido);
										dispose();
									...
									Util.alertMsg(Main.getVentana(),"Debe seleccionar un presupuesto de la grilla para poder duplicarlo");
									Main.getVentana().getGlassPane().stop();
								}						
							}	
						...
						Thread performer = new Thread(new Runnable(){
							public void run(){
								...
								try{
									...
									if(!pptoElegido.getVendedor().getVendedor().getCodUsuario().equals(getUser().getCodigo()) && getUser().getPerfil().equals("2")){
										Util.alertMsg(Main.getVentana(), "Usted no es el propietario del presupuesto.");
										loadPpto = false;
									}
									else{

										if (pptoElegido != null && pptoElegido.getActivo().equals("G")) {
											Util.alertMsg(Main.getVentana(), "El presupuesto esta siendo usado para cargar los datos de subcontrataciones.\nComuniquese con soporte tecnico.");
										}
