...
public class PantallaNuevoContactoLugar extends PantallaEmergente{
    ...
    private LugarEvento cliente;
    private LugarEventoContacto contacto;
    ...
    public PantallaNuevoContactoLugar(Frame owner, LugarEvento lugarId){		
		super("Nuevo contacto del lugar",owner);
        this.cliente = lugarId;
        createMenuBar();
	}
    ...
    public void init(){		
        ...
        jLabel7.setText(cliente.getNombreLugar());
    ...
    private void setAddMode() {
		this.contacto = new LugarEventoContacto();;
		resetFields();
	}
    
    public void setEditMode(String entityId) {
		try {			
			this.contacto = LugarEventoContactoManager.instance().getLugarEventoContactoById(entityId);
	...
	private void resetFields() {
		...
		if(contacto.getNextelFota() != null)
			flotaNextel.setText(contacto.getNextelFota().toString());
		if(contacto.getNextelId() != null)
			idNextel.setText(contacto.getNextelId().toString());
	}
	 ...
	 */
	public LugarEventoContacto getContacto() {
		return contacto;
	}
	 ...
	 */
	public void setContacto(LugarEventoContacto contacto) {
...
//************************************ACCIONES************************************************
	private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(PantallaNuevoContactoLugar.this, "¿Desea salir sin guardar cambios?", "Salir") == true){
				setVisible(false);
	...
	private class NuevoActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (formValid()){				
				try {			
		        	contacto.setCodLugar(cliente.getCodigo());
		            ...
		            contacto.setFecha(DateConverter.convertDateToString(new Date(),"yyyy/MM/dd H:mm:ss"));
		            ...
		            if(!flotaNextel.getText().equals(""))
		            	contacto.setNextelFota(flotaNextel.getText());
		            if(!idNextel.getText().equals(""))
		            	contacto.setNextelId(idNextel.getText());		            
		            
		            codContactoElegido = LugarEventoContactoManager.instance().update(contacto);	
		            
		            if(!StringUtils.isBlank(codContactoElegido))
		            	contacto.setCodigo(codContactoElegido);
		            
		            MessageUtil.showMessage(PantallaNuevoContactoLugar.this,"El contacto del lugar se cargó con éxito");

				} catch (RemoteException e) {			
					//MessageUtil.showErrorMessage(PantallaNuevoContactoCliente.this,"Error al guardar los datos");
					MessageUtil.showMessage(PantallaNuevoContactoLugar.this,"Error al cargar datos externos");
				}
				finally{
					setAddMode();
					setVisible(false);					
				...
				MessageUtil.showMessage(PantallaNuevoContactoLugar.this,"Faltan datos por completar");
			}
		}
