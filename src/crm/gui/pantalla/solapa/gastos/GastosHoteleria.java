...
import crm.gui.tablerenderer.gastos.GastosHoteleriaTableModel;
import crm.gui.tablerenderer.gastos.TableRenderGastosHoteleria;
import crm.libraries.abm.entities.Ppto_GastoHoteleria;
import crm.libraries.abm.entities.Presupuesto;
import crm.libraries.util.MessageUtil;

public class GastosHoteleria extends PanelGeneral {
    ...
    private TableRenderGastosHoteleria tableRender;
	private GastosHoteleriaItem item;
	private boolean editMode;
    
    private JPanel panel;
    private String detalleXDefault = "Indique en detalle el gasto de hotelería" ;
    
    public GastosHoteleria(JPanel pan){
    	panel = pan;
    }
    
    public void init(){
    	name = new javax.swing.JLabel();
        tableRender = new TableRenderGastosHoteleria();
        ...
        name.setText("Gastos de hotelería");
	...
	private void save(){
		makeItem();					
		if(!isEditMode()){		
			((GastosHoteleriaTableModel)this.tableRender.getTable().getModel()).addRow(this.item);
		}
	...
	public GastosHoteleriaItem getItem() {
		return item;
	}

	public void setItem(GastosHoteleriaItem item) {
		this.item = item;
	}
	...
	private void clearItem(){		
		this.item = new GastosHoteleriaItem();
		detalle.setText(detalleXDefault);
		setEditMode(false);
	}	
	
	public Ppto_GastoHoteleria[] getGastos(){		
		List items = ((GastosHoteleriaTableModel)this.tableRender.getTable().getModel()).getRows();
	
		Ppto_GastoHoteleria[] gastos = new Ppto_GastoHoteleria[items.size()];		
		for(int i = 0;i < gastos.length;i++){
			GastosHoteleriaItem item = (GastosHoteleriaItem)items.get(i);
			gastos[i] = new Ppto_GastoHoteleria();
	...
	public List getRows(){
		return ((GastosHoteleriaTableModel)this.tableRender.getTable().getModel()).getRows();
	}
	
	public void setPresupuesto(Presupuesto presupuesto){
		((GastosHoteleriaTableModel)this.tableRender.getTable().getModel()).clear();
		...
		if(gastos != null){
			...
			while (it.hasNext()) {
				...
				GastosHoteleriaItem item = new GastosHoteleriaItem();
				item.setCosto(Double.parseDouble(gasto.getCosto()));
				item.setDetalle(gasto.getDetalle());
				
				((GastosHoteleriaTableModel)this.tableRender.getTable().getModel()).addRow(item);
			}
		}
	}
	...
	private class AddActionListener implements ActionListener{		
		public void actionPerformed(ActionEvent e) {
			setEditMode(false);
			setItem(new GastosHoteleriaItem());
			makeItem();
			if(!isValid()){
