...
public class TableRenderGastosVarios extends TableRender {
	
	public TableRenderGastosVarios() {
		this(new ArrayList());
	}
	
	public TableRenderGastosVarios(List rows) {
		super(new GastosVariosTableModel(rows));
		...
		//setupDetalle(table.getColumnModel().getColumn(GastosVariosTableModel.COLUMNA_DETALLE));
	...
	public GastosVariosItem getSelectedItem(){
		
		// obtengo el modelo de la tabla
		GastosVariosTableModel model = (GastosVariosTableModel)getTable().getModel();
...
//		 obtengo el numero de linea seleccionada de la vista
		...
		GastosVariosItem item = model.getRow(selrow);	
	...
	public void removeSelection(){
		((GastosVariosTableModel)getTable().getModel()).removeRow(getSelectedItem());
		getTable().updateUI();
	}
