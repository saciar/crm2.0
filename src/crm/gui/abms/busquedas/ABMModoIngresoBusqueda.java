...
public class ABMModoIngresoBusqueda extends JDialog{
    ...
    public ABMModoIngresoBusqueda(Frame owner){
    	super(owner);
    	this.setTitle("Buscar Modos de ingreso de equipos");
	...
	public void initComponents() {
        ...
        jLabel1.setText("B\u00fasqueda de modos de ingreso de equipos");

        jLabel2.setFont(new java.awt.Font("Tahoma", 1, 11));
        jLabel2.setText("Buscar por:");

        buttonGroup1.add(jRadioButton1);
        jRadioButton1.setText("Nombre de modo de ingreso de equipos");
        jRadioButton1.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jRadioButton1.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jRadioButton1.setSelected(true);
        
        buttonGroup1.add(jRadioButton2);
        jRadioButton2.setText("C\u00f3digo de modo de ingreso de equipos");
    ...
    private void setCodigoText(){
    	...
    	jFormattedTextField2.setForeground(new java.awt.Color(153, 153, 153));
        jFormattedTextField2.setText("Ingrese el c\u00f3digo del modo de ingreso de equipos");
        jFormattedTextField2.setFont(new java.awt.Font("SansSerif", 2, 12));
    }
...
//*********************************ACCIONES*******************************************/
    ...
    private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(ABMModoIngresoBusqueda.this, "¿Desea salir del formulario de busqueda?", "Salir")){
    ...
    private class BorrarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(it != null){
				try {
					if (MessageUtil.showYesNoMessage(ABMModoIngresoBusqueda.this, "¿Seguro que desea borrar el modo de ingreso de equipos seleccionado?", "Borrar")){
						ModoIngEquiposManager.instance().remove(it.getCodigo());
						((BusquedaBasicTableModel)jTable1.getTable().getModel()).removeRow(it);
						jTable1.refreshTable();	
					}
				} catch (RemoteException e) {
					Util.errMsg(Main.getVentana(), "Error al borrar el modo de ingreso de equipos",e);
				}				
			}
    ...
    private class BuscarActionListener implements ActionListener{
    	...
    	public void actionPerformed(ActionEvent arg0) {
    		try{
    			ModoIngEquipos[] modos = null;
    			if(jRadioButton1.isSelected() && !StringUtils.isBlank(jFormattedTextField1.getText())){
    				modos  = ModoIngEquiposManager.instance().findByField("descripcion",jFormattedTextField1.getText());
    			}
    			else if(jRadioButton2.isSelected() && !StringUtils.isBlank(jFormattedTextField2.getText())){
    				ModoIngEquipos mod = ModoIngEquiposManager.instance().getById(jFormattedTextField2.getText());
    				if(mod != null){
    					modos = new ModoIngEquipos[1];
    					modos[0] = mod; 
    				}
    			}    			
    			else if(jRadioButton1.isSelected() && StringUtils.isBlank(jFormattedTextField1.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un nombre de modo de ingreso de equipos a buscar",null);
    			}
    			else if(jRadioButton2.isSelected() && StringUtils.isBlank(jFormattedTextField2.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un codigo de modo de ingreso de equipos a buscar",null);
    			}
    			
    			if(modos != null && modos.length >0){
    				...
    				for(int i=0; i<modos.length;i++){
    					...
    					item.setCodigo(modos[i].getCodigo());
    					item.setNombre(modos[i].getDescripcion());
    			...
    			Util.errMsg(Main.getVentana(), "Error al buscar el modo de ingreso de equipos",e);
    		}
    	}
