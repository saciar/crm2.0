...
import crm.gui.tablerenderer.abms.BusquedaResultadoSeguimientoItem;
import crm.gui.tablerenderer.abms.BusquedaResultadoSeguimientoTableModel;
import crm.gui.tablerenderer.abms.BusquedaResultadoSeguimientoTableRender;
import crm.libraries.abm.entities.ResultadoSeguimiento;
import crm.libraries.util.MessageUtil;

public class ABMResultadosSeguimientoBusqueda extends JDialog{
    ...
    private BusquedaResultadoSeguimientoTableRender jTable1;
    ...
    public ABMResultadosSeguimientoBusqueda(Frame owner){
    	super(owner);
    	this.setTitle("Buscar Resultados de seguimiento");
	...
	public void initComponents() {
        ...
        jTable1 = new BusquedaResultadoSeguimientoTableRender();
        ...
        jLabel1.setText("B\u00fasqueda de resultados de seguimiento");

        jLabel2.setFont(new java.awt.Font("Tahoma", 1, 11));
        jLabel2.setText("Buscar por:");

        buttonGroup1.add(jRadioButton1);
        jRadioButton1.setText("Nombre de resultados de seguimiento");
        jRadioButton1.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jRadioButton1.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jRadioButton1.setSelected(true);
        
        buttonGroup1.add(jRadioButton2);
        jRadioButton2.setText("Nombre de acción de seguimiento");
        jRadioButton2.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jRadioButton2.setMargin(new java.awt.Insets(0, 0, 0, 0));
        
        buttonGroup1.add(jRadioButton3);
        jRadioButton3.setText("C\u00f3digo de resultados de seguimiento");
...
//*********************************ACCIONES*******************************************/
    ...
    private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(ABMResultadosSeguimientoBusqueda.this, "¿Desea salir del formulario de busqueda?", "Salir")){
    ...
    private class EditarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			BusquedaResultadoSeguimientoItem it = jTable1.getSelectedItem();
			if(it != null){
				codEntidadElegido = it.getIdResultado();
				setVisible(false);
			}
    ...
    private class BorrarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(it != null){
				try {
					if (MessageUtil.showYesNoMessage(ABMResultadosSeguimientoBusqueda.this, "¿Seguro que desea borrar el resultado de seguimiento seleccionada?", "Borrar")){
						ResultadoSeguimientoManager.instance().remove(it.getCodigo());
						((BusquedaResultadoSeguimientoTableModel)jTable1.getTable().getModel()).removeRow(it);
						jTable1.refreshTable();	
					}
				} catch (RemoteException e) {
					Util.errMsg(Main.getVentana(), "Error al borrar el resultado de seguimiento",e);
				}				
			}
    ...
    private class BuscarActionListener implements ActionListener{
    	...
    	public void actionPerformed(ActionEvent arg0) {
    		try{
    			ResultadoSeguimiento[] resultados = null;
    			if(jRadioButton1.isSelected() && !StringUtils.isBlank(jFormattedTextField1.getText())){
    				resultados  = ResultadoSeguimientoManager.instance().findByField("descripcion",jFormattedTextField1.getText());
    			}
    			else if(jRadioButton3.isSelected() && !StringUtils.isBlank(jFormattedTextField3.getText())){
    				ResultadoSeguimiento res = ResultadoSeguimientoManager.instance().getResultadoSeguimientoById(jFormattedTextField3.getText());
    				if(res != null){
    					resultados = new ResultadoSeguimiento[1];
    					resultados[0] = res; 
    				}
    			}
    			else if(jRadioButton2.isSelected() && !StringUtils.isBlank(jComboBox1.searchForeign())){ 
    				//resultados  = (ResultadoSeguimiento[])ResultadoSeguimientoManager.instance().getResultadosReportByAccion(jComboBox1.searchForeign());   				
    				Object[] o  = ResultadoSeguimientoManager.instance().getResultadosReportByAccion(jComboBox1.searchForeign());
    				resultados = new ResultadoSeguimiento[o.length];
    				for(int i=0; i<o.length;i++){
    					Object[] row = (Object[])o[i];   					
    					ResultadoSeguimiento r = new ResultadoSeguimiento();
    					r.setCodSeguimiento(jComboBox1.searchForeign());
    					r.setCodigo(row[0].toString());
    					r.setDescripcion(row[1].toString());
    					resultados[i] = r;
    				}
    			}
    			else if(jRadioButton1.isSelected() && StringUtils.isBlank(jFormattedTextField1.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un nombre de resultado de seguimiento a buscar",null);
    			}
    			else if(jRadioButton3.isSelected() && StringUtils.isBlank(jFormattedTextField3.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un codigo de resultado de seguimiento a buscar",null);
    			}
    			else if(jRadioButton2.isSelected() && StringUtils.isBlank(jComboBox1.searchForeign())){ 
    				Util.errMsg(Main.getVentana(),"Seleccione una acción para buscar sus resultados de seguimiento",null);   				
    			}
    			
    			if(resultados != null && resultados.length >0){
    				BusquedaResultadoSeguimientoTableModel model = new BusquedaResultadoSeguimientoTableModel();
    				
    				for(int i=0; i<resultados.length;i++){
    					BusquedaResultadoSeguimientoItem item = new BusquedaResultadoSeguimientoItem();
    					item.setCodigo(resultados[i].getCodigo());
    					item.setIdResultado(resultados[i].getCodigo());
    					//item.setNombreAccion(PaisManager.instance().getNombrePaisById(provincias[i].getCodigoPais()));
    					item.setNombreAccion((SeguimientoManager.instance().getSeguimientoById(resultados[i].getCodSeguimiento())).getDescripcion());
    					item.setNombreResultado(resultados[i].getDescripcion());
    				...
    				((BusquedaResultadoSeguimientoTableModel)jTable1.getTable().getModel()).clear();
    				jTable1.refreshTable();	
    			}
    			
    		} catch (Exception e) {
    			Util.errMsg(Main.getVentana(), "Error al buscar el resultado de seguimiento",e);
    		}
    	}
