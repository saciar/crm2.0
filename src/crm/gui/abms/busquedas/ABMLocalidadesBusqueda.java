...
public class ABMLocalidadesBusqueda extends JDialog{
	...
	public ABMLocalidadesBusqueda (Frame owner){
		
		super(owner);
		this.setTitle("Busqueda de localidades");
			...
			//new Thread(new Runnable() {	
				//public void run() {	
					try {						
						...
						Util.errMsg(ABMLocalidadesBusqueda.this,"Error al cargar datos externos.",e);
						//ProgressDialogUtil.closeProcessDialog();
					}
				//}
			//}).start(); 
	...
	private class SalirAction implements ActionListener {
		public void actionPerformed (ActionEvent evt) {
			if (MessageUtil.showYesNoMessage(ABMLocalidadesBusqueda.this, "¿Desea salir sin grabar el cliente?", "Salir")){
	...
	private class SelectActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {			
			BuscadorLocalidadesItem it = jTable1.getSelectedItem();
			if(it != null){
				codEntidadElegido = it.getIdLocalidad();
				setVisible(false);
			}
	...
	private class BorrarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(it != null){
				try {
					if (MessageUtil.showYesNoMessage(ABMLocalidadesBusqueda.this, "¿Seguro que desea borrar la localidad seleccionada?", "Borrar")){
						LocalidadManager.instance().remove(it.getCodigo());
						((BuscadorLocalidadesTableModel)jTable1.getTable().getModel()).removeRow(it);
						jTable1.refreshTable();	
					}
				} catch (RemoteException e) {
					Util.errMsg(Main.getVentana(), "Error al borrar la localidad",e);
				}				
			}
