...
public class ABMCancelacionPptoBusqueda extends JDialog{
    ...
    public ABMCancelacionPptoBusqueda(Frame owner){
    	super(owner);
    	this.setTitle("Buscar Cancelación");
	...
	public void initComponents() {
        ...
        jLabel1.setText("B\u00fasqueda de cancelación");

        jLabel2.setFont(new java.awt.Font("Tahoma", 1, 11));
        jLabel2.setText("Buscar por:");

        buttonGroup1.add(jRadioButton1);
        jRadioButton1.setText("Nombre de cancelación");
        jRadioButton1.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jRadioButton1.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jRadioButton1.setSelected(true);
        
        buttonGroup1.add(jRadioButton2);
        jRadioButton2.setText("C\u00f3digo de cancelación");
    ...
    private void setCodigoText(){
    	...
    	jFormattedTextField2.setForeground(new java.awt.Color(153, 153, 153));
        jFormattedTextField2.setText("Ingrese el c\u00f3digo de la cancelación a buscar");
        jFormattedTextField2.setFont(new java.awt.Font("SansSerif", 2, 12));
    }
...
//*********************************ACCIONES*******************************************/
    ...
    private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(ABMCancelacionPptoBusqueda.this, "¿Desea salir del formulario de busqueda?", "Salir")){
    ...
    private class BorrarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(it != null){
				try {
					if (MessageUtil.showYesNoMessage(ABMCancelacionPptoBusqueda.this, "¿Seguro que desea borrar la cancelación seleccionado?", "Borrar")){
						PrtPptoCancelacionManager.instance().remove(it.getCodigo());
						((BusquedaBasicTableModel)jTable1.getTable().getModel()).removeRow(it);
						jTable1.refreshTable();	
					}
				} catch (RemoteException e) {
					Util.errMsg(Main.getVentana(), "Error al borrar la cancelación",e);
				}				
			}
    ...
    private class BuscarActionListener implements ActionListener{
    	...
    	public void actionPerformed(ActionEvent arg0) {
    		try{
    			PrtPptoCancelacion[] cancelaciones = null;
    			if(jRadioButton1.isSelected() && !StringUtils.isBlank(jFormattedTextField1.getText())){
    				cancelaciones  = PrtPptoCancelacionManager.instance().findByField("descripcion",jFormattedTextField1.getText());
    			}
    			else if(jRadioButton2.isSelected() && !StringUtils.isBlank(jFormattedTextField2.getText())){
    				PrtPptoCancelacion canc = PrtPptoCancelacionManager.instance().getById(jFormattedTextField2.getText());
    				if(canc != null){
    					cancelaciones = new PrtPptoCancelacion[1];
    					cancelaciones[0] = canc; 
    				}
    			}    			
    			else if(jRadioButton1.isSelected() && StringUtils.isBlank(jFormattedTextField1.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un nombre de cancelación a buscar",null);
    			}
    			else if(jRadioButton2.isSelected() && StringUtils.isBlank(jFormattedTextField2.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un codigo de cancelación a buscar",null);
    			}
    			
    			if(cancelaciones != null && cancelaciones.length >0){
    				...
    				for(int i=0; i<cancelaciones.length;i++){
    					...
    					item.setCodigo(cancelaciones[i].getCodigo());
    					item.setNombre(cancelaciones[i].getDescripcion());
    			...
    			Util.errMsg(Main.getVentana(), "Error al buscar la cancelación",e);
    		}
    	}
