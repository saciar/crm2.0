...
public class ABMCondicionesPagoBusqueda extends JDialog{
    ...
    public ABMCondicionesPagoBusqueda(Frame owner){
    	super(owner);
    	this.setTitle("Buscar Condición de Pago");
	...
	public void initComponents() {
        ...
        jLabel1.setText("B\u00fasqueda de condición de pago");

        jLabel2.setFont(new java.awt.Font("Tahoma", 1, 11));
        jLabel2.setText("Buscar por:");

        buttonGroup1.add(jRadioButton1);
        jRadioButton1.setText("Nombre de condición de pago");
        jRadioButton1.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jRadioButton1.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jRadioButton1.setSelected(true);
        
        buttonGroup1.add(jRadioButton2);
        jRadioButton2.setText("C\u00f3digo de condición de pago");
    ...
    private void setCodigoText(){
    	...
    	jFormattedTextField2.setForeground(new java.awt.Color(153, 153, 153));
        jFormattedTextField2.setText("Ingrese el c\u00f3digo de la condición de pago a buscar");
        jFormattedTextField2.setFont(new java.awt.Font("SansSerif", 2, 12));
    }
...
//*********************************ACCIONES*******************************************/
    ...
    private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(ABMCondicionesPagoBusqueda.this, "¿Desea salir del formulario de busqueda?", "Salir")){
    ...
    private class BorrarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(it != null){
				try {
					if (MessageUtil.showYesNoMessage(ABMCondicionesPagoBusqueda.this, "¿Seguro que desea borrar la condición de pago seleccionado?", "Borrar")){
						PrtPptoCondPagoManager.instance().remove(it.getCodigo());
						((BusquedaBasicTableModel)jTable1.getTable().getModel()).removeRow(it);
						jTable1.refreshTable();	
					}
				} catch (RemoteException e) {
					Util.errMsg(Main.getVentana(), "Error al borrar la condición de pago",e);
				}				
			}
    ...
    private class BuscarActionListener implements ActionListener{
    	...
    	public void actionPerformed(ActionEvent arg0) {
    		try{
    			PrtPptoCondPago[] condiciones = null;
    			if(jRadioButton1.isSelected() && !StringUtils.isBlank(jFormattedTextField1.getText())){
    				condiciones  = PrtPptoCondPagoManager.instance().findByField("descripcion",jFormattedTextField1.getText());
    			}
    			else if(jRadioButton2.isSelected() && !StringUtils.isBlank(jFormattedTextField2.getText())){
    				PrtPptoCondPago cond = PrtPptoCondPagoManager.instance().getById(jFormattedTextField2.getText());
    				if(cond != null){
    					condiciones = new PrtPptoCondPago[1];
    					condiciones[0] = cond; 
    				}
    			}    			
    			else if(jRadioButton1.isSelected() && StringUtils.isBlank(jFormattedTextField1.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un nombre de condición de pago a buscar",null);
    			}
    			else if(jRadioButton2.isSelected() && StringUtils.isBlank(jFormattedTextField2.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un codigo de condición de pago a buscar",null);
    			}
    			...
    			if(condiciones != null && condiciones.length >0){
    				...
    				for(int i=0; i<condiciones.length;i++){
    					...
    					item.setNombre(condiciones[i].getTitulo());
    			...
    			Util.errMsg(Main.getVentana(), "Error al buscar la condición de pago",e);
    		}
    	}
