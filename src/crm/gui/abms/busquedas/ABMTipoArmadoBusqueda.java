...
public class ABMTipoArmadoBusqueda extends JDialog{
    ...
    public ABMTipoArmadoBusqueda(Frame owner){
    	super(owner);
    	this.setTitle("Buscar Tipos de Armados");
	...
	public void initComponents() {
        ...
        jLabel1.setText("B\u00fasqueda de tipos de armado");

        jLabel2.setFont(new java.awt.Font("Tahoma", 1, 11));
        jLabel2.setText("Buscar por:");

        buttonGroup1.add(jRadioButton1);
        jRadioButton1.setText("Nombre de tipo de armado");
        jRadioButton1.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jRadioButton1.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jRadioButton1.setSelected(true);
        
        buttonGroup1.add(jRadioButton2);
        jRadioButton2.setText("C\u00f3digo de tipo de armado");
    ...
    private void setCodigoText(){
    	...
    	jFormattedTextField2.setForeground(new java.awt.Color(153, 153, 153));
        jFormattedTextField2.setText("Ingrese el c\u00f3digo del tipo de armado a buscar");
        jFormattedTextField2.setFont(new java.awt.Font("SansSerif", 2, 12));
    }
...
//*********************************ACCIONES*******************************************/
    ...
    private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(ABMTipoArmadoBusqueda.this, "¿Desea salir del formulario de busqueda?", "Salir")){
    ...
    private class BorrarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(it != null){
				try {
					if (MessageUtil.showYesNoMessage(ABMTipoArmadoBusqueda.this, "¿Seguro que desea borrar el tipo de armado seleccionado?", "Borrar")){
						TipoArmadoManager.instance().remove(it.getCodigo());
						((BusquedaBasicTableModel)jTable1.getTable().getModel()).removeRow(it);
						jTable1.refreshTable();	
					}
				} catch (RemoteException e) {
					Util.errMsg(Main.getVentana(), "Error al borrar el tipo de armado",e);
				}				
			}
    ...
    private class BuscarActionListener implements ActionListener{
    	...
    	public void actionPerformed(ActionEvent arg0) {
    		try{
    			TipoArmado[] tipoArmado = null;
    			if(jRadioButton1.isSelected() && !StringUtils.isBlank(jFormattedTextField1.getText())){
    				tipoArmado  = TipoArmadoManager.instance().findByField("descripcion",jFormattedTextField1.getText());
    			}
    			else if(jRadioButton2.isSelected() && !StringUtils.isBlank(jFormattedTextField2.getText())){
    				TipoArmado tipo = TipoArmadoManager.instance().getById(jFormattedTextField2.getText());
    				if(tipo != null){
    					tipoArmado = new TipoArmado[1];
    					tipoArmado[0] = tipo; 
    				}
    			}    			
    			else if(jRadioButton1.isSelected() && StringUtils.isBlank(jFormattedTextField1.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un nombre de tipo de armado a buscar",null);
    			}
    			else if(jRadioButton2.isSelected() && StringUtils.isBlank(jFormattedTextField2.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un codigo de tipo de armado a buscar",null);
    			}
    			
    			if(tipoArmado != null && tipoArmado.length >0){
    				BusquedaBasicTableModel model = new BusquedaBasicTableModel();
    				
    				for(int i=0; i<tipoArmado.length;i++){
    					BusquedaBasicItem item = new BusquedaBasicItem();
    					item.setCodigo(tipoArmado[i].getCodigo());
    					item.setNombre(tipoArmado[i].getDescripcion());
    			...
    			Util.errMsg(Main.getVentana(), "Error al buscar el tipo de armado",e);
    		}
    	}
