...
public class ABMEstadosEventoBusqueda extends JDialog{
    ...
    public ABMEstadosEventoBusqueda(Frame owner){
    	super(owner);
    	this.setTitle("Buscar Estados de Evento");
...
public URL getUrlImagen(String imagen){
	...
	public void initComponents() {
        ...
        jLabel1.setText("B\u00fasqueda de estados de evento");

        jLabel2.setFont(new java.awt.Font("Tahoma", 1, 11));
        jLabel2.setText("Buscar por:");

        buttonGroup1.add(jRadioButton1);
        jRadioButton1.setText("Nombre de estado de evento");
        jRadioButton1.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jRadioButton1.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jRadioButton1.setSelected(true);
        
        buttonGroup1.add(jRadioButton2);
        jRadioButton2.setText("C\u00f3digo de estado de evento");
    ...
    private void setCodigoText(){
    	...
    	jFormattedTextField2.setForeground(new java.awt.Color(153, 153, 153));
        jFormattedTextField2.setText("Ingrese el c\u00f3digo del estado de evento a buscar");
        jFormattedTextField2.setFont(new java.awt.Font("SansSerif", 2, 12));
    }
...
//*********************************ACCIONES*******************************************/
    ...
    private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(ABMEstadosEventoBusqueda.this, "¿Desea salir del formulario de busqueda?", "Salir")){
    ...
    private class BorrarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(it != null){
				try {
					if (MessageUtil.showYesNoMessage(ABMEstadosEventoBusqueda.this, "¿Seguro que desea borrar el estado de evento seleccionado?", "Borrar")){
						EstadoEventoManager.instance().remove(it.getCodigo());
						((BusquedaBasicTableModel)jTable1.getTable().getModel()).removeRow(it);
						jTable1.refreshTable();	
					}
				} catch (RemoteException e) {
					Util.errMsg(Main.getVentana(), "Error al borrar el estado de evento",e);
				}				
			}
    ...
    private class BuscarActionListener implements ActionListener{
    	...
    	public void actionPerformed(ActionEvent arg0) {
    		try{
    			EstadoEvento[] estados = null;
    			if(jRadioButton1.isSelected() && !StringUtils.isBlank(jFormattedTextField1.getText())){
    				estados  = EstadoEventoManager.instance().findByField("descripcion",jFormattedTextField1.getText());
    			}
    			else if(jRadioButton2.isSelected() && !StringUtils.isBlank(jFormattedTextField2.getText())){
    				EstadoEvento est = EstadoEventoManager.instance().getEstadoEventoById(jFormattedTextField2.getText());
    				if(est != null){
    					estados = new EstadoEvento[1];
    					estados[0] = est; 
    				}
    			}    			
    			else if(jRadioButton1.isSelected() && StringUtils.isBlank(jFormattedTextField1.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un nombre de estado de evento a buscar",null);
    			}
    			else if(jRadioButton2.isSelected() && StringUtils.isBlank(jFormattedTextField2.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un codigo de estado de evento a buscar",null);
    			}
    			...
    			if(estados != null && estados.length >0){
    				...
    				for(int i=0; i<estados.length;i++){
    					...
    					item.setCodigo(estados[i].getCodigo());
    					item.setNombre(estados[i].getDescripcion());
    			...
    			Util.errMsg(Main.getVentana(), "Error al buscar el estado de evento",e);
    		}
    	}
