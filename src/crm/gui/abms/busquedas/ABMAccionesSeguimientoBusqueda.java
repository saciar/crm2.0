...
public class ABMAccionesSeguimientoBusqueda extends JDialog{
    ...
    public ABMAccionesSeguimientoBusqueda(Frame owner){
    	super(owner);
    	this.setTitle("Buscar Acciones de Seguimientos");
	...
	public void initComponents() {
        ...
        jLabel1.setText("B\u00fasqueda de acciones de seguimientos");

        jLabel2.setFont(new java.awt.Font("Tahoma", 1, 11));
        jLabel2.setText("Buscar por:");

        buttonGroup1.add(jRadioButton1);
        jRadioButton1.setText("Nombre de acciones de seguimientos");
        jRadioButton1.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jRadioButton1.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jRadioButton1.setSelected(true);
        
        buttonGroup1.add(jRadioButton2);
        jRadioButton2.setText("C\u00f3digo de acciones de seguimientos");
    ...
    private void setCodigoText(){
    	...
    	jFormattedTextField2.setForeground(new java.awt.Color(153, 153, 153));
        jFormattedTextField2.setText("Ingrese el c\u00f3digo de la accion de seguimiento a buscar");
        jFormattedTextField2.setFont(new java.awt.Font("SansSerif", 2, 12));
    }
...
//*********************************ACCIONES*******************************************/
    ...
    private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(ABMAccionesSeguimientoBusqueda.this, "¿Desea salir del formulario de busqueda?", "Salir")){
    ...
    private class BorrarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(it != null){
				try {
					if (MessageUtil.showYesNoMessage(ABMAccionesSeguimientoBusqueda.this, "¿Seguro que desea borrar la accion de seguimiento seleccionada?", "Borrar")){
						SeguimientoManager.instance().remove(it.getCodigo());
						((BusquedaBasicTableModel)jTable1.getTable().getModel()).removeRow(it);
						jTable1.refreshTable();	
					}
				} catch (RemoteException e) {
					Util.errMsg(Main.getVentana(), "Error al borrar la accion de seguimiento",e);
				}				
			}
    ...
    private class BuscarActionListener implements ActionListener{
    	...
    	public void actionPerformed(ActionEvent arg0) {
    		try{
    			Seguimiento[] acciones = null;
    			if(jRadioButton1.isSelected() && !StringUtils.isBlank(jFormattedTextField1.getText())){
    				acciones  = SeguimientoManager.instance().findByField("descripcion",jFormattedTextField1.getText());
    			}
    			else if(jRadioButton2.isSelected() && !StringUtils.isBlank(jFormattedTextField2.getText())){
    				Seguimiento acc = SeguimientoManager.instance().getSeguimientoById(jFormattedTextField2.getText());
    				if(acc != null){
    					acciones = new Seguimiento[1];
    					acciones[0] = acc; 
    				}
    			}    			
    			else if(jRadioButton1.isSelected() && StringUtils.isBlank(jFormattedTextField1.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un nombre de la accion de seguimiento a buscar",null);
    			}
    			else if(jRadioButton2.isSelected() && StringUtils.isBlank(jFormattedTextField2.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un codigo de la accion de seguimiento a buscar",null);
    			}
    			...
    			if(acciones != null && acciones.length >0){
    				...
    				for(int i=0; i<acciones.length;i++){
    					...
    					item.setCodigo(acciones[i].getCodigo());
    					item.setNombre(acciones[i].getDescripcion());
    			...
    			Util.errMsg(Main.getVentana(), "Error al buscar la accion de seguimiento",e);
    		}
    	}
