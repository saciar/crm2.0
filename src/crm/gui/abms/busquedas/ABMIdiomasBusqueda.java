...
public class ABMIdiomasBusqueda extends JDialog{
    ...
    public ABMIdiomasBusqueda(Frame owner){
    	super(owner);
    	this.setTitle("Buscar Idiomas");
...
public URL getUrlImagen(String imagen){
	...
	public void initComponents() {
        ...
        jLabel1.setText("B\u00fasqueda de idiomas");

        jLabel2.setFont(new java.awt.Font("Tahoma", 1, 11));
        jLabel2.setText("Buscar por:");

        buttonGroup1.add(jRadioButton1);
        jRadioButton1.setText("Nombre de idioma");
        jRadioButton1.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jRadioButton1.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jRadioButton1.setSelected(true);
        
        buttonGroup1.add(jRadioButton2);
        jRadioButton2.setText("C\u00f3digo de idioma");
    ...
    private void setCodigoText(){
    	...
    	jFormattedTextField2.setForeground(new java.awt.Color(153, 153, 153));
        jFormattedTextField2.setText("Ingrese el c\u00f3digo del idioma a buscar");
        jFormattedTextField2.setFont(new java.awt.Font("SansSerif", 2, 12));
    }
...
//*********************************ACCIONES*******************************************/
    ...
    private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(ABMIdiomasBusqueda.this, "¿Desea salir del formulario de busqueda?", "Salir")){
    ...
    private class BorrarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(it != null){
				try {
					if (MessageUtil.showYesNoMessage(ABMIdiomasBusqueda.this, "¿Seguro que desea borrar el idioma seleccionado?", "Borrar")){
						IdiomaManager.instance().remove(it.getCodigo());
						((BusquedaBasicTableModel)jTable1.getTable().getModel()).removeRow(it);
						jTable1.refreshTable();	
					}
				} catch (RemoteException e) {
					Util.errMsg(Main.getVentana(), "Error al borrar el idioma",e);
				}				
			}
    	...
    	public void actionPerformed(ActionEvent arg0) {
    		try{
    			...
    			if(jRadioButton1.isSelected() && !StringUtils.isBlank(jFormattedTextField1.getText())){
    				idiomas  = IdiomaManager.instance().findByField("descripcion",jFormattedTextField1.getText());
    			}
    			else if(jRadioButton2.isSelected() && !StringUtils.isBlank(jFormattedTextField2.getText())){
    				Idioma id = IdiomaManager.instance().getIdiomaById(jFormattedTextField2.getText());
    				if(id != null){
    					idiomas = new Idioma[1];
    					idiomas[0] = id; 
    				}
    			}    			
    			else if(jRadioButton1.isSelected() && StringUtils.isBlank(jFormattedTextField1.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un nombre de idioma a buscar",null);
    			}
    			else if(jRadioButton2.isSelected() && StringUtils.isBlank(jFormattedTextField2.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un codigo de idioma a buscar",null);
    			}
    			...
    			if(idiomas != null && idiomas.length >0){
    				...
    				for(int i=0; i<idiomas.length;i++){
    					...
    					item.setCodigo(idiomas[i].getCodigo());
    					item.setNombre(idiomas[i].getDescripcion());
    			...
    			Util.errMsg(Main.getVentana(), "Error al buscar el idioma",e);
    		}
    	}
