...
public class ABMCategoriasVendedoresBusqueda extends JDialog{
    ...
    public ABMCategoriasVendedoresBusqueda(Frame owner){
    	super(owner);
    	this.setTitle("Buscar Categorias de Vendedores");
	...
	public void initComponents() {
        ...
        jLabel1.setText("B\u00fasqueda de categorias de vendedores");

        jLabel2.setFont(new java.awt.Font("Tahoma", 1, 11));
        jLabel2.setText("Buscar por:");

        buttonGroup1.add(jRadioButton1);
        jRadioButton1.setText("Nombre de categoria de vendedor");
        jRadioButton1.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jRadioButton1.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jRadioButton1.setSelected(true);
        
        buttonGroup1.add(jRadioButton2);
        jRadioButton2.setText("C\u00f3digo de categoria de vendedor");
    ...
    private void setCodigoText(){
    	...
    	jFormattedTextField2.setForeground(new java.awt.Color(153, 153, 153));
        jFormattedTextField2.setText("Ingrese el c\u00f3digo de la categoria de vendedor a buscar");
        jFormattedTextField2.setFont(new java.awt.Font("SansSerif", 2, 12));
    }
...
//*********************************ACCIONES*******************************************/
    ...
    private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(ABMCategoriasVendedoresBusqueda.this, "¿Desea salir del formulario de busqueda?", "Salir")){
    ...
    private class BorrarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(it != null){
				try {
					if (MessageUtil.showYesNoMessage(ABMCategoriasVendedoresBusqueda.this, "¿Seguro que desea borrar la categoria de evento seleccionado?", "Borrar")){
						CategVendedorManager.instance().remove(it.getCodigo());
						((BusquedaBasicTableModel)jTable1.getTable().getModel()).removeRow(it);
						jTable1.refreshTable();	
					}
				} catch (RemoteException e) {
					Util.errMsg(Main.getVentana(), "Error al borrar la categoria de vendedor",e);
				}				
			}
    ...
    private class BuscarActionListener implements ActionListener{
    	...
    	public void actionPerformed(ActionEvent arg0) {
    		try{
    			CategVendedor[] categorias = null;
    			if(jRadioButton1.isSelected() && !StringUtils.isBlank(jFormattedTextField1.getText())){
    				categorias  = CategVendedorManager.instance().findByField("descripcion",jFormattedTextField1.getText());
    			}
    			else if(jRadioButton2.isSelected() && !StringUtils.isBlank(jFormattedTextField2.getText())){
    				CategVendedor cat = CategVendedorManager.instance().getCategVendedorById(jFormattedTextField2.getText());
    				if(cat != null){
    					categorias = new CategVendedor[1];
    					categorias[0] = cat; 
    				}
    			}    			
    			else if(jRadioButton1.isSelected() && StringUtils.isBlank(jFormattedTextField1.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un nombre de categoria de vendedor a buscar",null);
    			}
    			else if(jRadioButton2.isSelected() && StringUtils.isBlank(jFormattedTextField2.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un codigo de categoria de vendedor a buscar",null);
    			}
    			...
    			if(categorias != null && categorias.length >0){
    				...
    				for(int i=0; i<categorias.length;i++){
    					...
    					item.setCodigo(categorias[i].getCodigo());
    					item.setNombre(categorias[i].getDescripcion());
    			...
    			Util.errMsg(Main.getVentana(), "Error al buscar la categoria de vendedor",e);
    		}
    	}
