...
public class ABMPerfilesBusqueda extends JDialog{
    ...
    public ABMPerfilesBusqueda(Frame owner){
    	super(owner);
    	this.setTitle("Buscar Perfil");
...
public URL getUrlImagen(String imagen){
	...
	public void initComponents() {
        ...
        jLabel1.setText("B\u00fasqueda de perfiles");

        jLabel2.setFont(new java.awt.Font("Tahoma", 1, 11));
        jLabel2.setText("Buscar por:");

        buttonGroup1.add(jRadioButton1);
        jRadioButton1.setText("Nombre de perfil");
        jRadioButton1.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jRadioButton1.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jRadioButton1.setSelected(true);
        
        buttonGroup1.add(jRadioButton2);
        jRadioButton2.setText("C\u00f3digo de perfil");
    ...
    private void setCodigoText(){
    	...
    	jFormattedTextField2.setForeground(new java.awt.Color(153, 153, 153));
        jFormattedTextField2.setText("Ingrese el c\u00f3digo del perfil a buscar");
        jFormattedTextField2.setFont(new java.awt.Font("SansSerif", 2, 12));
    }
...
//*********************************ACCIONES*******************************************/
    ...
    private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(ABMPerfilesBusqueda.this, "¿Desea salir del formulario de busqueda?", "Salir")){
    ...
    private class BorrarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(it != null){
				try {
					if (MessageUtil.showYesNoMessage(ABMPerfilesBusqueda.this, "¿Seguro que desea borrar el perfil seleccionado?", "Borrar")){
						PerfilManager.instance().remove(it.getCodigo());
						((BusquedaBasicTableModel)jTable1.getTable().getModel()).removeRow(it);
						jTable1.refreshTable();	
					}
				} catch (RemoteException e) {
					Util.errMsg(Main.getVentana(), "Error al borrar el perfil",e);
				}				
			}
    	...
    	public void actionPerformed(ActionEvent arg0) {
    		try{
    			...
    			if(jRadioButton1.isSelected() && !StringUtils.isBlank(jFormattedTextField1.getText())){
    				accesos  = PerfilManager.instance().findByField("descripcion",jFormattedTextField1.getText());
    			}
    			else if(jRadioButton2.isSelected() && !StringUtils.isBlank(jFormattedTextField2.getText())){
    				Perfil acc = PerfilManager.instance().getPerfilById(jFormattedTextField2.getText());
    				if(acc != null){
    					accesos = new Perfil[1];
    					accesos[0] = acc; 
    				}
    			}    			
    			else if(jRadioButton1.isSelected() && StringUtils.isBlank(jFormattedTextField1.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un nombre de perfil a buscar",null);
    			}
    			else if(jRadioButton2.isSelected() && StringUtils.isBlank(jFormattedTextField2.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un codigo de perfil a buscar",null);
    			}
    			...
    			if(accesos != null && accesos.length >0){
    				...
    				for(int i=0; i<accesos.length;i++){
    					...
    					item.setCodigo(accesos[i].getCodigo());
    					item.setNombre(accesos[i].getDescripcion());
    			...
    			Util.errMsg(Main.getVentana(), "Error al buscar el perfil",e);
    		}
    	}
