...
import crm.gui.tablerenderer.abms.BusquedaProveedoresItem;
import crm.gui.tablerenderer.abms.BusquedaProveedoresTableModel;
import crm.gui.tablerenderer.abms.BusquedaProveedoresTableRender;
import crm.libraries.abm.entities.Proveedor;
import crm.libraries.util.MessageUtil;

public class ABMProveedoresBusqueda extends JDialog{
    ...
    private BusquedaProveedoresTableRender jTable1;
    ...
    public ABMProveedoresBusqueda(Frame owner){
    	super(owner);
	...
	public void initComponents() {
        ...
        jTable1 = new BusquedaProveedoresTableRender();
        ...
        jLabel1.setText("B\u00fasqueda de proveedor");
        ...
        jRadioButton2.setText("C\u00f3digo del proveedor");
	...
	private void createListeners(){
    	...
    	jRadioButton1.addChangeListener(new NombreProveedoresChangeListener());
    	jRadioButton2.addChangeListener(new CodigoChangeListener());
    	
    	jFormattedTextField1.addKeyListener(new NombreProveedoresKeyListener());
    	jFormattedTextField1.addMouseListener(new NombreProveedoresMouseListener());
    ...
    private void setCodigoText(){
    	...
    	jFormattedTextField2.setForeground(new java.awt.Color(153, 153, 153));
        jFormattedTextField2.setText("Ingrese el c\u00f3digo del proveedor a buscar");
        jFormattedTextField2.setFont(new java.awt.Font("SansSerif", 2, 12));
    }
...
//*********************************ACCIONES*******************************************/
    ...
    private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(ABMProveedoresBusqueda.this, "¿Desea salir del formulario de busqueda?", "Salir")){
    ...
    private class EditarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			BusquedaProveedoresItem it = jTable1.getSelectedItem();
			if(it != null){
    ...
    private class BorrarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(it != null){
				try {
					if (MessageUtil.showYesNoMessage(ABMProveedoresBusqueda.this, "¿Seguro que desea borrar el proveedor seleccionado?", "Borrar")){
						ProveedorManager.instance().remove(it.getCodigo());
						((BusquedaProveedoresTableModel)jTable1.getTable().getModel()).removeRow(it);
						jTable1.refreshTable();	
					}
				} catch (RemoteException e) {
					Util.errMsg(Main.getVentana(), "Error al borrar el proveedor",e);
				}				
			}
    	...
    	public void actionPerformed(ActionEvent arg0) {
    		try{
    			...
    			if(jRadioButton1.isSelected() && !StringUtils.isBlank(jFormattedTextField1.getText())){
    				proveedores  = ProveedorManager.instance().findByField("nombre",jFormattedTextField1.getText());
    			}
    			else if(jRadioButton2.isSelected() && !StringUtils.isBlank(jFormattedTextField2.getText())){
    				Proveedor prov = ProveedorManager.instance().getProveedorById(jFormattedTextField2.getText());
    				if(prov != null){
    				...
    				Util.errMsg(Main.getVentana(),"Ingrese un nombre del proveedor a buscar",null);
    			}
    			else if(jRadioButton2.isSelected() && StringUtils.isBlank(jFormattedTextField2.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un codigo del proveedor a buscar",null);
    			}
    			
    			if(proveedores != null && proveedores.length >0){
    				BusquedaProveedoresTableModel model = new BusquedaProveedoresTableModel();
    				
    				for(int i=0; i<proveedores.length;i++){
    					BusquedaProveedoresItem item = new BusquedaProveedoresItem();
    					item.setCodigo(proveedores[i].getCodigo());
    					item.setNombre(proveedores[i].getNombre());
    					item.setContacto(proveedores[i].getContacto());
    				...
    				((BusquedaProveedoresTableModel)jTable1.getTable().getModel()).clear();
    				jTable1.refreshTable();	
    			}
    			
    		} catch (Exception e) {
    			Util.errMsg(Main.getVentana(), "Error al buscar el proveedor",e);
    		}
    	}
    	
    }
    
    private class NombreProveedoresKeyListener extends KeyAdapter{
    ...
    private class NombreProveedoresChangeListener implements ChangeListener{
    ...
    private class NombreProveedoresMouseListener extends MouseAdapter{
