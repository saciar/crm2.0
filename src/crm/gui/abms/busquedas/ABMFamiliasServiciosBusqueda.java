...
public class ABMFamiliasServiciosBusqueda extends JDialog{
    ...
    public ABMFamiliasServiciosBusqueda(Frame owner){
    	super(owner);
    	this.setTitle("Buscar Familias de servicios");
	...
	public void initComponents() {
        ...
        jLabel1.setText("B\u00fasqueda de familia de servicios");

        jLabel2.setFont(new java.awt.Font("Tahoma", 1, 11));
        jLabel2.setText("Buscar por:");

        buttonGroup1.add(jRadioButton1);
        jRadioButton1.setText("Nombre de familia");
        jRadioButton1.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jRadioButton1.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jRadioButton1.setSelected(true);
        
        buttonGroup1.add(jRadioButton2);
        jRadioButton2.setText("C\u00f3digo de familia");
    ...
    private void setCodigoText(){
    	...
    	jFormattedTextField2.setForeground(new java.awt.Color(153, 153, 153));
        jFormattedTextField2.setText("Ingrese el c\u00f3digo de familia a buscar");
        jFormattedTextField2.setFont(new java.awt.Font("SansSerif", 2, 12));
    }
...
//*********************************ACCIONES*******************************************/
    ...
    private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(ABMFamiliasServiciosBusqueda.this, "¿Desea salir del formulario de busqueda?", "Salir")){
    ...
    private class BorrarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(it != null){
				try {
					if (MessageUtil.showYesNoMessage(ABMFamiliasServiciosBusqueda.this, "¿Seguro que desea borrar la familia seleccionada?", "Borrar")){
						FamiliaServManager.instance().remove(it.getCodigo());
						((BusquedaBasicTableModel)jTable1.getTable().getModel()).removeRow(it);
						jTable1.refreshTable();	
					}
				} catch (RemoteException e) {
					Util.errMsg(Main.getVentana(), "Error al borrar la familia de servicio",e);
				}				
			}
    ...
    private class BuscarActionListener implements ActionListener{
    	...
    	public void actionPerformed(ActionEvent arg0) {
    		try{
    			FamiliaServ[] familias = null;
    			if(jRadioButton1.isSelected() && !StringUtils.isBlank(jFormattedTextField1.getText())){
    				familias  = FamiliaServManager.instance().findByField("descripcion",jFormattedTextField1.getText());
    			}
    			else if(jRadioButton2.isSelected() && !StringUtils.isBlank(jFormattedTextField2.getText())){
    				FamiliaServ suc = FamiliaServManager.instance().getFamiliaServById(jFormattedTextField2.getText());
    				if(suc != null){
    					familias = new FamiliaServ[1];
    					familias[0] = suc; 
    				}
    			}    			
    			else if(jRadioButton1.isSelected() && StringUtils.isBlank(jFormattedTextField1.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un nombre de familia a buscar",null);
    			}
    			else if(jRadioButton2.isSelected() && StringUtils.isBlank(jFormattedTextField2.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un codigo de familia a buscar",null);
    			}
    			...
    			if(familias != null && familias.length >0){
    				...
    				for(int i=0; i<familias.length;i++){
    					...
    					item.setCodigo(familias[i].getCodigo());
    					item.setNombre(familias[i].getDescripcion());
    			...
    			Util.errMsg(Main.getVentana(), "Error al buscar la familia de servicios",e);
    		}
    	}
