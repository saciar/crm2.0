...
public class ABMTipoLugarBusqueda extends JDialog{
    ...
    public ABMTipoLugarBusqueda(Frame owner){
    	super(owner);
    	this.setTitle("Buscar Tipos de Lugares de Evento");
...
public URL getUrlImagen(String imagen){
	...
	public void initComponents() {
        ...
        jLabel1.setText("B\u00fasqueda de tipos de evento");

        jLabel2.setFont(new java.awt.Font("Tahoma", 1, 11));
        jLabel2.setText("Buscar por:");

        buttonGroup1.add(jRadioButton1);
        jRadioButton1.setText("Nombre de tipo de lugar de evento");
        jRadioButton1.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jRadioButton1.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jRadioButton1.setSelected(true);
        
        buttonGroup1.add(jRadioButton2);
        jRadioButton2.setText("C\u00f3digo de tipo de lugar de evento");
    ...
    private void setCodigoText(){
    	...
    	jFormattedTextField2.setForeground(new java.awt.Color(153, 153, 153));
        jFormattedTextField2.setText("Ingrese el c\u00f3digo del tipo de lugar de evento a buscar");
        jFormattedTextField2.setFont(new java.awt.Font("SansSerif", 2, 12));
    }
...
//*********************************ACCIONES*******************************************/
    ...
    private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(ABMTipoLugarBusqueda.this, "¿Desea salir del formulario de busqueda?", "Salir")){
    ...
    private class BorrarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(it != null){
				try {
					if (MessageUtil.showYesNoMessage(ABMTipoLugarBusqueda.this, "¿Seguro que desea borrar el tipo de lugar de evento seleccionado?", "Borrar")){
						TipoLugarEventoManager.instance().remove(it.getCodigo());
						((BusquedaBasicTableModel)jTable1.getTable().getModel()).removeRow(it);
						jTable1.refreshTable();	
					}
				} catch (RemoteException e) {
					Util.errMsg(Main.getVentana(), "Error al borrar el tipo de lugar de evento",e);
				}				
			}
    ...
    private class BuscarActionListener implements ActionListener{
    	...
    	public void actionPerformed(ActionEvent arg0) {
    		try{
    			TipoLugarEvento[] tipoLugares = null;
    			if(jRadioButton1.isSelected() && !StringUtils.isBlank(jFormattedTextField1.getText())){
    				tipoLugares  = TipoLugarEventoManager.instance().findByField("descripcion",jFormattedTextField1.getText());
    			}
    			else if(jRadioButton2.isSelected() && !StringUtils.isBlank(jFormattedTextField2.getText())){
    				TipoLugarEvento tipo = TipoLugarEventoManager.instance().getTipoLugarEventoById(jFormattedTextField2.getText());
    				if(tipo != null){
    					tipoLugares = new TipoLugarEvento[1];
    					tipoLugares[0] = tipo; 
    				}
    			}    			
    			else if(jRadioButton1.isSelected() && StringUtils.isBlank(jFormattedTextField1.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un nombre de tipo de lugar de evento a buscar",null);
    			}
    			else if(jRadioButton2.isSelected() && StringUtils.isBlank(jFormattedTextField2.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un codigo de tipo de lugar de evento a buscar",null);
    			}
    			
    			if(tipoLugares != null && tipoLugares.length >0){
    				BusquedaBasicTableModel model = new BusquedaBasicTableModel();
    				
    				for(int i=0; i<tipoLugares.length;i++){
    					BusquedaBasicItem item = new BusquedaBasicItem();
    					item.setCodigo(tipoLugares[i].getCodigo());
    					item.setNombre(tipoLugares[i].getDescripcion());
    			...
    			Util.errMsg(Main.getVentana(), "Error al buscar el tipo de lugar de evento",e);
    		}
    	}
