...
import crm.gui.tablerenderer.abms.BusquedaUsuariosItem;
import crm.gui.tablerenderer.abms.BusquedaUsuariosTableModel;
import crm.gui.tablerenderer.abms.BusquedaUsuariosTableRender;
import crm.libraries.abm.entities.Usuario;
import crm.libraries.util.MessageUtil;

public class ABMUsuariosBusqueda extends JDialog{
    ...
    private BusquedaUsuariosTableRender jTable1;
    ...
    public ABMUsuariosBusqueda(Frame owner){
    	super(owner);
...
public URL getUrlImagen(String imagen){
	...
	public void initComponents() {
        ...
        jTable1 = new BusquedaUsuariosTableRender();
        ...
        jRadioButton1.setText("Nombre del usuario");
        jRadioButton1.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jRadioButton1.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jRadioButton1.setSelected(true);

        buttonGroup1.add(jRadioButton2);
        jRadioButton2.setText("C\u00f3digo del usuariol");
	...
	private void createListeners(){
    	...
    	jRadioButton1.addChangeListener(new NombreUsuariosChangeListener());
    	jRadioButton2.addChangeListener(new CodigoChangeListener());
    	
    	jFormattedTextField1.addKeyListener(new NombreUsuariosKeyListener());
    	jFormattedTextField1.addMouseListener(new NombreUsuariosMouseListener());
    ...
    private void setCodigoText(){
    	...
    	jFormattedTextField2.setForeground(new java.awt.Color(153, 153, 153));
        jFormattedTextField2.setText("Ingrese el c\u00f3digo del usuario a buscar");
        jFormattedTextField2.setFont(new java.awt.Font("SansSerif", 2, 12));
    }
...
//*********************************ACCIONES*******************************************/
    ...
    private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(ABMUsuariosBusqueda.this, "¿Desea salir del formulario de busqueda?", "Salir")){
    ...
    private class EditarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			BusquedaUsuariosItem it = jTable1.getSelectedItem();
			if(it != null){
    ...
    private class BorrarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(it != null){
				try {
					if (MessageUtil.showYesNoMessage(ABMUsuariosBusqueda.this, "¿Seguro que desea borrar el usuario seleccionada?", "Borrar")){
						UsuarioManager.instance().remove(it.getCodigo());
						((BusquedaUsuariosTableModel)jTable1.getTable().getModel()).removeRow(it);
						jTable1.refreshTable();	
					}
				} catch (RemoteException e) {
					Util.errMsg(Main.getVentana(), "Error al borrar el usuario",e);
				}				
			}
    	...
    	public void actionPerformed(ActionEvent arg0) {
    		try{
    			...
    			if(jRadioButton1.isSelected() && !StringUtils.isBlank(jFormattedTextField1.getText())){
    				usuarios  = UsuarioManager.instance().findByField("apellidoYNombre",jFormattedTextField1.getText());
    			}
    			else if(jRadioButton2.isSelected() && !StringUtils.isBlank(jFormattedTextField2.getText())){
    				Usuario suc = UsuarioManager.instance().getUsuarioById(jFormattedTextField2.getText());
    				if(suc != null){
    				...
    				Util.errMsg(Main.getVentana(),"Ingrese un nombre del usuario a buscar",null);
    			}
    			else if(jRadioButton2.isSelected() && StringUtils.isBlank(jFormattedTextField2.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un codigo del usuario a buscar",null);
    			}
    			
    			if(usuarios != null && usuarios.length >0){
    				BusquedaUsuariosTableModel model = new BusquedaUsuariosTableModel();
    				
    				for(int i=0; i<usuarios.length;i++){
    					BusquedaUsuariosItem item = new BusquedaUsuariosItem();
    					item.setCodigo(usuarios[i].getCodigo());
    					item.setNombre(usuarios[i].getApellidoYNombre());
    					item.setLogin(usuarios[i].getLoginName());
    				...
    				((BusquedaUsuariosTableModel)jTable1.getTable().getModel()).clear();
    				jTable1.refreshTable();	
    			}
    			
    		} catch (Exception e) {
    			Util.errMsg(Main.getVentana(), "Error al buscar el usuario",e);
    		}
    	}
    	
    }
    
    private class NombreUsuariosKeyListener extends KeyAdapter{
    ...
    private class NombreUsuariosChangeListener implements ChangeListener{
    ...
    private class NombreUsuariosMouseListener extends MouseAdapter{
