...
import crm.gui.tablerenderer.abms.BusquedaUnidadesComercialesItem;
import crm.gui.tablerenderer.abms.BusquedaUnidadesComercialesTableModel;
import crm.gui.tablerenderer.abms.BusquedaUnidadesComercialesTableRender;
import crm.libraries.abm.entities.UnidadAdministrativa;
import crm.libraries.util.MessageUtil;

public class ABMUnidadesAdministrativasBusqueda extends JDialog{
    ...
    private BusquedaUnidadesComercialesTableRender jTable1;
    ...
    public ABMUnidadesAdministrativasBusqueda(Frame owner){
    	super(owner);
    	this.setTitle("Buscar Unidades Administrativas");
...
public URL getUrlImagen(String imagen){
	...
	public void initComponents() {
        ...
        jTable1 = new BusquedaUnidadesComercialesTableRender();
        ...
        jLabel1.setText("B\u00fasqueda de unidad administrativa");

        jLabel2.setFont(new java.awt.Font("Tahoma", 1, 11));
        jLabel2.setText("Buscar por:");

        buttonGroup1.add(jRadioButton1);
        jRadioButton1.setText("Nombre de unidad administrativa");
        jRadioButton1.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jRadioButton1.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jRadioButton1.setSelected(true);
        
        buttonGroup1.add(jRadioButton2);
        jRadioButton2.setText("C\u00f3digo de unidad administrativa");
    ...
    private void setCodigoText(){
    	...
    	jFormattedTextField2.setForeground(new java.awt.Color(153, 153, 153));
        jFormattedTextField2.setText("Ingrese el c\u00f3digo de unidad administrativa a buscar");
        jFormattedTextField2.setFont(new java.awt.Font("SansSerif", 2, 12));
    }
...
//*********************************ACCIONES*******************************************/
    ...
    private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(ABMUnidadesAdministrativasBusqueda.this, "¿Desea salir del formulario de busqueda?", "Salir")){
    ...
    private class EditarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			BusquedaUnidadesComercialesItem it = jTable1.getSelectedItem();
			if(it != null){
    ...
    private class BorrarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(it != null){
				try {
					if (MessageUtil.showYesNoMessage(ABMUnidadesAdministrativasBusqueda.this, "¿Seguro que desea borrar la unidad administrativa seleccionada?", "Borrar")){
						UnidadAdministrativaManager.instance().remove(it.getCodigo());
						UnidadAdministradorManager.instance().removeByCodigoUnidad(it.getCodigo());
						((BusquedaUnidadesComercialesTableModel)jTable1.getTable().getModel()).removeRow(it);
						jTable1.refreshTable();	
					}
				} catch (RemoteException e) {
					Util.errMsg(Main.getVentana(), "Error al borrar la unidad administrativa",e);
				}				
			}
    ...
    private class BuscarActionListener implements ActionListener{
    	...
    	public void actionPerformed(ActionEvent arg0) {
    		try{
    			UnidadAdministrativa[] unidades = null;
    			if(jRadioButton1.isSelected() && !StringUtils.isBlank(jFormattedTextField1.getText())){
    				unidades  = UnidadAdministrativaManager.instance().findByField("descripcion",jFormattedTextField1.getText());
    			}
    			else if(jRadioButton2.isSelected() && !StringUtils.isBlank(jFormattedTextField2.getText())){
    				UnidadAdministrativa uni = UnidadAdministrativaManager.instance().getUnidadComercialById(jFormattedTextField2.getText());
    				if(uni != null){
    					unidades = new UnidadAdministrativa[1];
    					unidades[0] = uni; 
    				}
    			}    			
    			else if(jRadioButton1.isSelected() && StringUtils.isBlank(jFormattedTextField1.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un nombre de unidad administrativa a buscar",null);
    			}
    			else if(jRadioButton2.isSelected() && StringUtils.isBlank(jFormattedTextField2.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un codigo de unidad administrativa a buscar",null);
    			}
    			
    			if(unidades != null && unidades.length >0){
    				BusquedaUnidadesComercialesTableModel model = new BusquedaUnidadesComercialesTableModel();
    				
    				for(int i=0; i<unidades.length;i++){
    					BusquedaUnidadesComercialesItem item = new BusquedaUnidadesComercialesItem();
    					item.setCodigo(unidades[i].getCodigo());
    					item.setUnidad(unidades[i].getDescripcion());
    				...
    				((BusquedaUnidadesComercialesTableModel)jTable1.getTable().getModel()).clear();
    				jTable1.refreshTable();	
    			}
    			
    		} catch (Exception e) {
    			Util.errMsg(Main.getVentana(), "Error al buscar la unidad administrativa",e);
    		}
    	}
