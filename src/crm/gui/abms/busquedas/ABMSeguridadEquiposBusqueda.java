...
public class ABMSeguridadEquiposBusqueda extends JDialog{
    ...
    public ABMSeguridadEquiposBusqueda(Frame owner){
    	super(owner);
    	this.setTitle("Buscar Seguridad de equipos");
	...
	public void initComponents() {
        ...
        jLabel1.setText("B\u00fasqueda de seguridad de equipos");

        jLabel2.setFont(new java.awt.Font("Tahoma", 1, 11));
        jLabel2.setText("Buscar por:");

        buttonGroup1.add(jRadioButton1);
        jRadioButton1.setText("Nombre de seguridad de equipos");
        jRadioButton1.setBorder(javax.swing.BorderFactory.createEmptyBorder(0, 0, 0, 0));
        jRadioButton1.setMargin(new java.awt.Insets(0, 0, 0, 0));
        jRadioButton1.setSelected(true);
        
        buttonGroup1.add(jRadioButton2);
        jRadioButton2.setText("C\u00f3digo de seguridad de equipos");
    ...
    private void setCodigoText(){
    	...
    	jFormattedTextField2.setForeground(new java.awt.Color(153, 153, 153));
        jFormattedTextField2.setText("Ingrese el c\u00f3digo de la seguridad de equipos");
        jFormattedTextField2.setFont(new java.awt.Font("SansSerif", 2, 12));
    }
...
//*********************************ACCIONES*******************************************/
    ...
    private class SalirActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			if (MessageUtil.showYesNoMessage(ABMSeguridadEquiposBusqueda.this, "¿Desea salir del formulario de busqueda?", "Salir")){
    ...
    private class BorrarActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(it != null){
				try {
					if (MessageUtil.showYesNoMessage(ABMSeguridadEquiposBusqueda.this, "¿Seguro que desea borrar la seguridad de equipos seleccionada?", "Borrar")){
						SeguridadEquiposManager.instance().remove(it.getCodigo());
						((BusquedaBasicTableModel)jTable1.getTable().getModel()).removeRow(it);
						jTable1.refreshTable();	
					}
				} catch (RemoteException e) {
					Util.errMsg(Main.getVentana(), "Error al borrar la seguridad de equipos",e);
				}				
			}
    ...
    private class BuscarActionListener implements ActionListener{
    	...
    	public void actionPerformed(ActionEvent arg0) {
    		try{
    			SeguridadEquipos[] seguridades = null;
    			if(jRadioButton1.isSelected() && !StringUtils.isBlank(jFormattedTextField1.getText())){
    				seguridades  = SeguridadEquiposManager.instance().findByField("descripcion",jFormattedTextField1.getText());
    			}
    			else if(jRadioButton2.isSelected() && !StringUtils.isBlank(jFormattedTextField2.getText())){
    				SeguridadEquipos seg = SeguridadEquiposManager.instance().getById(jFormattedTextField2.getText());
    				if(seg != null){
    					seguridades = new SeguridadEquipos[1];
    					seguridades[0] = seg; 
    				}
    			}    			
    			else if(jRadioButton1.isSelected() && StringUtils.isBlank(jFormattedTextField1.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un nombre de seguridad de equipos a buscar",null);
    			}
    			else if(jRadioButton2.isSelected() && StringUtils.isBlank(jFormattedTextField2.getText())){
    				Util.errMsg(Main.getVentana(),"Ingrese un codigo de seguridad de equipos a buscar",null);
    			}
    			
    			if(seguridades != null && seguridades.length >0){
    				BusquedaBasicTableModel model = new BusquedaBasicTableModel();
    				
    				for(int i=0; i<seguridades.length;i++){
    					BusquedaBasicItem item = new BusquedaBasicItem();
    					item.setCodigo(seguridades[i].getCodigo());
    					item.setNombre(seguridades[i].getDescripcion());
    			...
    			Util.errMsg(Main.getVentana(), "Error al buscar la seguridad de equipos",e);
    		}
    	}
