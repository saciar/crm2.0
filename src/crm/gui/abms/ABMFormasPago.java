...
import crm.gui.Main;
import crm.gui.abms.busquedas.ABMFormasPagoBusqueda;
import crm.libraries.abm.entities.PrtPptoFPago;

public class ABMFormasPago extends ABMGeneral{
	...
	private PrtPptoFPago model;
	
	public ABMFormasPago(JPanel pan){
		panel = pan;	
	}
    ...
    public void initComponents() {
        ...
        jLabel1.setText("Creaci\u00f3n de una nueva forma de pago");

        jLabel7.setIcon(new javax.swing.ImageIcon(getUrlImagen("DibujoFormaPago.jpg")));

        jLabel2.setText("Ingrese el titulo de la forma de pago");
    ...
    public void guardar() {        
        ...
        try {			        	
        	...
        	PrtPptoFPagoManager.instance().update(this.model);			
    ...
    private void setAddMode(){
    	this.model = new PrtPptoFPago();    	
    	this.resetFields();
    }
    
    public void setEditMode(String entityId){
    	 try {    		  		
    		 this.model = PrtPptoFPagoManager.instance().getById(entityId);    	
    		 this.resetFields();
    	 }catch (RemoteException e) {		
			e.printStackTrace();
		}
    ...
    public void buscar(){
    	ABMFormasPagoBusqueda busq = new ABMFormasPagoBusqueda(Main.getVentana());
		busq.initComponents();
