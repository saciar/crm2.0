...
import crm.gui.abms.busquedas.ABMPerfilesBusqueda;
import crm.libraries.abm.entities.Perfil;

public class ABMPerfiles extends ABMGeneral{
	...
	public void initComponents() {
        ...
        jLabel1.setText("Creaci\u00f3n de un perfil");

        jLabel2.setText("Ingrese el nombre del perfil");
	...
	private void setAddMode() {
		this.perfil = new Perfil("","","S");
		resetFields();
	}
	
	public void setEditMode(String entityId) {
		try {
			this.perfil = PerfilManager.instance().getPerfilById(entityId);
			resetFields();
	...
	private void resetFields() {
		jFormattedTextField1.setText(perfil.getDescripcion());
	}
	
	private boolean isFormIncomplete() {
		String nombrePerfil = jFormattedTextField1.getText();
		
		return nombrePerfil == null || nombrePerfil.length() == 0;
	}
	...
	public void guardar() {
		...
		this.perfil.setDescripcion(nombrePerfil);
		try {
			PerfilManager.instance().update(this.perfil);
			Util.alertMsg(Main.getVentana(), "El perfil se cargó con éxito");
		} catch (Exception e) {
			Util.errMsg(Main.getVentana(), "Error al guardar el perfil",e);
		} finally {
			setAddMode();
		}
	}
	...
	public void buscar(){
		ABMPerfilesBusqueda busq = new ABMPerfilesBusqueda(Main.getVentana());
		busq.initComponents();
