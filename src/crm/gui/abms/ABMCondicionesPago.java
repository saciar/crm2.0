...
import crm.gui.Main;
import crm.gui.abms.busquedas.ABMCondicionesPagoBusqueda;
import crm.gui.abms.busquedas.ABMEncabezadosPptoBusqueda;
import crm.libraries.abm.entities.PrtPptoCondPago;

public class ABMCondicionesPago extends ABMGeneral{
	...
	private PrtPptoCondPago model;
	
	public ABMCondicionesPago(JPanel pan){
		panel = pan;	
	}
    ...
    public void initComponents() {
        ...
        jLabel1.setText("Creaci\u00f3n de una nueva condición de pago");

        jLabel7.setIcon(new javax.swing.ImageIcon(getUrlImagen("DibujoCondicion.jpg")));

        jLabel2.setText("Ingrese el titulo de la condición de pago");
    ...
    public void guardar() {        
        ...
        try {			        	
			...
			PrtPptoCondPagoManager.instance().update(this.model);			
    ...
    private void setAddMode(){
    	this.model = new PrtPptoCondPago();    	
    	this.resetFields();
    }
    
    public void setEditMode(String entityId){
    	 try {    		  		
    		 this.model = PrtPptoCondPagoManager.instance().getById(entityId);    	
    		 this.resetFields();
    	 }catch (RemoteException e) {		
			e.printStackTrace();
		}
    ...
    public void buscar(){
    	ABMCondicionesPagoBusqueda busq = new ABMCondicionesPagoBusqueda(Main.getVentana());
		busq.initComponents();
