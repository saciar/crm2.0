...
import crm.gui.abms.busquedas.ABMUnidadNegocioBusqueda;
import crm.libraries.abm.entities.UnidadNegocio;

public class ABMUnidadDeNegocio extends ABMGeneral{
	...
	public ABMUnidadDeNegocio(JPanel pan){
		panel = pan;	
	}
	...
	public void initComponents() {
        ...
        jLabel1.setText("Creaci\u00f3n de una nueva unidad de negocio");

        jLabel2.setText("Ingrese el nombre de la unidad de negocio");
	...
	private void setAddMode() {
		this.unidadNegocio = new UnidadNegocio("","","S");
		resetFields();
	}
	
	public void setEditMode(String entityId) {
		try {
			this.unidadNegocio = UnidadNegocioManager.instance().getUnidadNegocioById(entityId);
			resetFields();
	...
	private void resetFields() {
		jFormattedTextField1.setText(unidadNegocio.getDescripcion());
	}
	
	private boolean isFormIncomplete() {
		String unidad = jFormattedTextField1.getText();
		
		return unidad == null || unidad.length() == 0;
	}
	...
	public void guardar() {
		...
		this.unidadNegocio.setDescripcion(familiaServicio);
		try {
			UnidadNegocioManager.instance().update(this.unidadNegocio);
			Util.alertMsg(Main.getVentana(), "La unidad de negocio se cargó con éxito");
		} catch (RemoteException e) {
			Util.errMsg(Main.getVentana(), "Error al guardar la unidad de negocio",e);
		} finally {
			setAddMode();
		}
	}
	...
	public void buscar(){
		ABMUnidadNegocioBusqueda busq = new ABMUnidadNegocioBusqueda(Main.getVentana());
		busq.initComponents();
