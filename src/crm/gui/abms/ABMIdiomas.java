...
import crm.gui.Main;
import crm.gui.abms.busquedas.ABMIdiomasBusqueda;
import crm.libraries.abm.entities.Idioma;

public class ABMIdiomas extends ABMGeneral{
	...
	public void initComponents() {
        ...
        jLabel1.setText("Creaci\u00f3n de un Idioma");

        jLabel2.setText("Ingrese el nombre del idioma");
	...
	private void setAddMode() {
		this.idioma = new Idioma("","","S");
		resetFields();
	}
	
	public void setEditMode(String entityId) {
		try {
			this.idioma = IdiomaManager.instance().getIdiomaById(entityId);
			resetFields();
	...
	private void resetFields() {
		jFormattedTextField1.setText(idioma.getDescripcion());
	}
	...
	public void guardar() {
		...
		this.idioma.setDescripcion(nombreTitulo);
		try {
			IdiomaManager.instance().update(this.idioma);
			Util.alertMsg(Main.getVentana(), "El idioma se cargó con éxito");
		} catch (Exception e) {
			Util.errMsg(Main.getVentana(), "Error al guardar el idioma",e);
		} finally {
			setAddMode();
		}
	}
	...
	public void buscar(){
		ABMIdiomasBusqueda busq = new ABMIdiomasBusqueda(Main.getVentana());
		busq.initComponents();
